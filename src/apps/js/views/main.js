...
(function(_, joint) {
    ...
    App.MainView = joint.mvc.View.extend({
        ...
        initializePaper: function() {
            ...
            this.$('.chart-container').append(paperScroller.el);
            paperScroller.render().center();
        },

        // Create and populate stencil.
        initializeStencil: function() {
delete App.config.stencil.groups.uml;
delete App.config.stencil.groups.org;
delete App.config.stencil.groups.pn;
delete App.config.stencil.groups.erd;
            ...
            stencil.options.layout.resizeToFit=false;
            stencil.options.layout.columns=9;
            stencil.options.layout.columnWidth= 50;
        ...
        initializeKeyboardShortcuts: function() {
            ...
            this.keyboard.on({
                ...
                'ctrl+c': function() {
                    ...
                    var elements = _.filter(this.selection.collection.models, function(cell) {
                        if(cell.isElement()){
                            copyObject.copyNodes.push(cell.id);
                            copyObject.parentVId=(currentSelectedNode.id).split("_")[1];
                            return true;
                        }
                        return false;
                    });
                ...
                'ctrl+v': function() {
                    ...
                    var elements = _.filter(pastedCells, function(cell) {
                        if(cell.isElement()){
                            copyObject.pastedNodes.push(cell.id);
                            var index=copyObject.pastedNodes.length-1;
                            saveTestData(copyObject.copyNodes[index],cell.id);                            
                            return true;
                        }
                        return false;
                    });
        ...
        initializeHaloAndInspector: function() {
            ...
            this.paper.on('element:pointerup link:options', function(cellView) {
                $("#scenariosTable").css("height","70px");
                ...
                if (!this.selection.collection.contains(cell)) {
                    ...
                    if (cell.isElement()) {
                        ...
                        new joint.ui.FreeTransform({
                         ...
                         var haloObject=new joint.ui.Halo({
                            cellView: cellView,
                        ...
                        haloObject.on('action:clone:pointerdown', function(evt, x, y) {
                            var nodeId=this.options.cellView.model.id;
                            this.startCloning(evt, x, y);
                            var clonedNodeId=this._cloneView.model.id;
                            copyObject.clonedNodes[clonedNodeId]=nodeId;  
                            saveTestData(nodeId,clonedNodeId);                           
                        });
                        haloObject.on('action:fork:pointerdown', function(evt, x, y) {
                            var nodeId=this.options.cellView.model.id;
                            this.startForking(evt, x, y);
                            var clonedNodeId=this._cloneView.model.id;
                            copyObject.clonedNodes[clonedNodeId]=nodeId;  
                            saveTestData(nodeId,clonedNodeId);                           
                        });
        ...
        initializeNavigator: function() {
            ...
            navigator.render();
            this.$('.navigator1-container').append(navigator.el);
        },
        ...
        initializeToolbar: function () {
            ...
            $('#btn-layout').addClass('icon-btn-disable');
            $('#btn-fullscreen').addClass('icon-btn-disable');
            $('#btn-zoom-to-fit').addClass('icon-btn-disable');
            $('.zoomtofitbrackets').addClass('icon-btn-disable');
            $('#btn-zoom-out').addClass('icon-btn-disable');
            $('#btn-zoom-in').addClass('icon-btn-disable');
            $('#btn-zoom-slider-label').addClass('icon-btn-disable');
            $('#btn-zoom-slider').addClass('icon-btn-disable');
